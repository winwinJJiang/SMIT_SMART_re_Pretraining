...
from data_loader3D_mask_ASM_Teacher import Dataset3D,Dataset3D_No_Intensity_Aug,Dataset3D_Jue_Custmzed,Dataset3D_Jue_Custmzed_CT_and_MRI,Dataset3D_Jue_Custmzed_CT_and_MRI_Not_Square,Dataset3D_Jue_Custmzed_CT_Not_Square
...
def train_one_epoch3D(student, teacher, teacher_without_ddp, trainloss, data_loader,
    ...
    for it, subjects_batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        #images_and_masks = [im.cuda(non_blocking=True) for im in subjects_batch]
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            #print ('images[:2] ',images[:2].size())
            #print ('masks_teacher size ',masks_teacher[0].shape)
            #print ('mask_token size ',mask_token[0].shape)
            ...
            if use_att_mask:
                ...
                #print ('computed_mask_ratio ',computed_mask_ratio)

                #print ('computed mask_token size is ',mask_token.shape)
                masks=nn.functional.interpolate(mask_token.unsqueeze(0), scale_factor=8, mode="nearest").cpu().numpy()
            ...
            image_rec_loss=student_output_all[2]*args.img_rec_weight
