...
class iBOTHead_w_Rec(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        x_rec_img=x_all[2]
        if len(x.shape) == 2:
            return super(iBOTHead_w_Rec, self).forward(x)
        ...
        #print('x1 ',x1)
        #print('x2 ',x2) # x2 is none 
        return x1, x2 ,x_rec,x_rec_img



class iBOTHead_w_Rec_Half(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_Half, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        if self.last_layer is not None:
            ...
            #print('x after is ',x)
            #x=x.half()
            #print('before norm ',x) # x2 is none 
            x=x.float()
            x = nn.functional.normalize(x, dim=-1, p=2)
            x=x.half()
            #print('after norm ',x) # x2 is none 

            x1 = self.last_layer(x[:, 0])
            #print('x[:, 1:] ',x[:, 1:]) # x2 is none 
            x2 = self.last_layer2(x[:, 1:])
            #print('x2 ',x2) # x2 is none 
        else:
            x = self.mlp[:-1](x)
...
class iBOTHead_w_2_loss_Rec(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_2_loss_Rec, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        if self.last_layer is not None:
            ...
            x = self.mlp(x)
            x = nn.functional.normalize(x, dim=-1, p=2)
            x1 = self.last_layer(x[:, 0:2])
            x2 = self.last_layer2(x[:, 2:])
        else:
            x = self.mlp[:-1](x)
            x1 = self.mlp[-1](x[:, 0:2])
            x2 = self.mlp2(x[:, 2:])
...
class iBOTHead_w_Rec_Att(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_Att, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        return x1, x2 ,x_rec,x_rec_img,x_att




class iBOTHead_w_Rec_2_Loss_Att_ViT(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_2_Loss_Att_ViT, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        x=x_all[0]
        x_rec=x_all[1] #x_rec loss
        x_rec_img=x_all[2] #x_rec image
        x_att=x_all[3] # x_att
        if len(x.shape) == 2:
            return super(iBOTHead_w_Rec, self).forward(x)
        ...
        if self.last_layer is not None:
            ...
            x = self.mlp(x)
            x = nn.functional.normalize(x, dim=-1, p=2)
            x1 = self.last_layer(x[:, 0:1]) #2 channel
            x2 = self.last_layer2(x[:, 1:]) 
        else:
            x = self.mlp[:-1](x)
            x1 = self.mlp[-1](x[:, 0:1]) #2 channel
            x2 = self.mlp2(x[:, 1:])
        
        if self.last_norm is not None:
            x1 = self.last_norm(x1)
            x2 = self.last_norm2(x2)
        #print ('info: x1 shape ',x1.shape)
        #print ('info: x2 shape ',x2.shape)
        #print ('info: x_att size ',x_att.size())
        return x1, x2 ,x_rec,x_rec_img,x_att


class iBOTHead_w_Rec_2_Loss_Att(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_2_Loss_Att, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        if self.last_layer is not None:
            ...
            x = self.mlp(x)
            x = nn.functional.normalize(x, dim=-1, p=2)
            x1 = self.last_layer(x[:, 0:2]) #2 channel
            x2 = self.last_layer2(x[:, 2:]) 
        else:
            x = self.mlp[:-1](x)
            x1 = self.mlp[-1](x[:, 0:2]) #2 channel
            x2 = self.mlp2(x[:, 2:])
...
class iBOTHead_w_Rec_2_Loss_Att_SMART_half(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_2_Loss_Att_SMART_half, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
    ...
    def forward(self, x_all):
        ...
        #print ('len x_all ',len(x_all))
        # x_region_all , att_map , x_rec
        x=x_all[0]
        x_rec_img=x_all[1] #x_rec image
        x_att=x_all[2] # x_att
        ...
        #print ('info: x1 shape ',x1.shape)
        #print ('info: x2 shape ',x2.shape)
        return x1, x2 ,x_rec_img,x_att


class iBOTHead_w_Rec_2_Loss_Att_Half(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True, 
                 shared_head=False, **kwargs):
        
        super(iBOTHead_w_Rec_2_Loss_Att_Half, self).__init__(*args,
                                        norm=norm,
                                        act=act,
                                        last_norm=last_norm,
                                        nlayers=nlayers,
...
class Head_CLS_Rec(nn.Module):
    ...
    def forward(self, x_all):
        ...
        x = nn.functional.normalize(x0, dim=-1, p=2)
        x = self.last_layer(x)
        return x,x_rec,x_rec_img,x_att
