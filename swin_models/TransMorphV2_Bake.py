...
class PatchMerging_V2(nn.Module):
    ...
    def __init__(self, dim, out_dim=None, norm_layer=nn.LayerNorm ,reduce_factor=2):
        """
        Args:
            dim (int): Number of input channels.
    ...
    def forward(self, x, H, W, T):
        ...
        #print ('error x1 before resahpe ',x.shape)
        ...
        B, H, W, T,C = x.shape
        x = x.view(B, -1,  C)  # B H/2*W/2*T/2 8*C
        #print ('error: final after patch merging size ',x.shape)
...
def swinv2_3D_tiny_window4_128(pretrained=False):
    ...
    model = Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss(
        config)
