...
class SwinTransformerBlock_V2_norm(nn.Module):
    r""" Swin Transformer Block.
    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads.
...
class BasicLayer_v2_norm(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of feature channels
        depth (int): Depths of this stage.
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_v2_norm(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = BasicLayer_v2_norm(dim=int(embed_dim * 2 ** i_layer),
                                depth=depths[i_layer],
                                num_heads=num_heads[i_layer],
                                window_size=window_size,
                                mlp_ratio=mlp_ratio,
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_debug(nn.Module):
    ...
    def forward(self, x_in, mask):
        ...
        #print ('error: mask size ',mask.max())
        mask_intp = mask.repeat_interleave(2, 1).repeat_interleave(2, 2).repeat_interleave(2, 3).unsqueeze(1).contiguous()
        #print ('error: mask_intp size ',mask_intp.max())
        x_in=x_in.float()
        x_rec=x_rec.float()
        mask_intp=mask_intp.float()

        loss_recon = F.l1_loss(x_in, x_rec, reduction='none')
        #print ('error: loss_recon ',loss_recon)
        #loss = (loss_recon * mask_intp).sum() / (mask_intp.sum() + 1e-5) 
        loss = (loss_recon * mask_intp).sum() / (mask_intp.sum() + 1e-5) 

        #loss1 = (loss_recon * mask_intp).sum()
        #loss2= mask_intp.sum()
        #print ('error: loss ',loss)
        #print ('error: loss1 ',loss1)
        #print ('error: loss2 ',loss2)

        #loss=loss.half()
        #print ('error: mask size ',mask_intp.max())
        #print ('error: loss size ',loss)
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_debug_v2_norm(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        self.transformer = SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_v2_norm(patch_size=config.patch_size, 
                                           in_chans=config.in_chans,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
                                           num_heads=config.num_heads,
