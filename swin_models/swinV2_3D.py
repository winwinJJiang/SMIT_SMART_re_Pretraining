...
class SwinTransformerV2Stage(nn.Module):
    ...
    def forward(self, x):
        print ('error: current dim is ',self.dim)
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss(nn.Module):
    ...
    def forward(self, x,mask,mask_flag):
        ...
        return x3_feature, cls_tokens, x4_last,x3_feature#x3_feature, x3_feature_w_cls_1
...
class SwinTransformerV2_MIM(nn.Module):
    ...
    def __init__(
        ...
        self.feat_size= [int(128/self.encoder_stride),int(128/self.encoder_stride),int(128/self.encoder_stride)]
    ...
    def forward_features(self, x_in,mask):
        ...
        print ('info: after all layer x size ',x_reshape.shape)
        x = self.norm(x)
        return x
...
def swinv2_3D_tiny_window4_96(pretrained=False, **kwargs):
    ...
    model = SwinTransformerV2_MIM(
       img_size=128,window_size=4, embed_dim=48,patch_size=2, depths=(2,2,18), num_heads=(4,4,8),qkv_bias=True, **kwargs)
