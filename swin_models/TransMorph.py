...
class WindowAttention_Half(nn.Module):
    """ Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
...
class LayerScale_Block_CA(nn.Module):
    ...
    def forward(self, x, x_cls):
        ...
        #print ('x cls size ',x_cls.shape) #10,1,1024
        #print ('x  size ',x.shape) #10,576,1024
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss(nn.Module):
    ...
    def forward(self, x,mask,mask_flag):
        ...
        #print ('info: x3_feature size ',x3_feature.shape)
        cls_tokens = self.cls_token.expand(B_size, -1, -1)  
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_2(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        embed_dim_CaiT=192 # from Stage 2 use
        #embed_dim_CaiT=768  # this one now try to enlarge the feature size
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim_CaiT))
        self.blocks_token_only = nn.ModuleList([
             LayerScale_Block_CA(
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_1(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        embed_dim_CaiT=192 # from Stage 2 use
        embed_dim_CaiT=96 # from Stage 1 use
        #embed_dim_CaiT=768  # this one now try to enlarge the feature size
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim_CaiT))
        self.blocks_token_only = nn.ModuleList([
             LayerScale_Block_CA(
...
class SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_4(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        embed_dim_CaiT=192 # from Stage 2 use
        embed_dim_CaiT=768  # from Stage 4 use
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim_CaiT))
        self.blocks_token_only = nn.ModuleList([
             LayerScale_Block_CA(
                dim=embed_dim_CaiT, num_heads=num_heads_CaiT, mlp_ratio=4.0, qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class TransMorph_Swin_Contrastive(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        self.contrastive_head = nn.Linear(embed_dim, 512)
    ...
    def forward(self, x_in):
        ...
        x = self.contrastive_pre(x[:, 1])
        x = self.contrastive_head(x)
        
        
        
        
        print ('info: constras x size is  ',x.size())
        return x
...
CONFIGS = {
    ...
    'TransMorph-Large_SSIM_pre_train_128_middle_bias_True_Swin_T':configs.get_3DTransMorphMiddle_config_Unetr_128_bias_True_Swin_T(), 
    'TransMorph-Large_SSIM_pre_train_128_middle_bias_True_Swin_S':configs.get_3DTransMorphMiddle_config_Unetr_128_bias_True_Swin_S(), 
    'TransMorph-Large_SSIM_pre_train_128_middle_bias_True_Swin_B':configs.get_3DTransMorphMiddle_config_Unetr_128_bias_True_Swin_B(), 
    'TransMorph-Large_SSIM_pre_train_128_middle_bias_True_Swin_L':configs.get_3DTransMorphMiddle_config_Unetr_128_bias_True_Swin_L(), 
    ...
    'Aneesh_smit_train':configs.get_SMIT_Aneesh_96_bias_True(),  
...
class Trans_SMIT_pre_train_cls_patch_rec_Student(nn.Module):
    def __init__(
        ...
        #super(TransMorph_Unetr, self).__init__()
        super().__init__()
        self.hidden_size = config.embed_dim*8
        self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        #self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        ...
        #print ('info: hidden_size is ',hidden_size)

        cls_norm=partial(nn.LayerNorm, eps=1e-5)

        self.norm_cls = cls_norm(self.hidden_size*2)
        self.norm_patch = cls_norm(self.hidden_size)
        ...
        # for reconstruction loss use
        self.encoder_stride=32
        self.decoder1 = nn.Conv3d(self.hidden_size*2,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1)

        
    def proj_feat(self, x, hidden_size, feat_size):

        #print ('info: x size is ',x.shape)
        #print ('info: self.hidden_size size is ',self.hidden_size)
        #print ('info: self.feat_size size is ',self.feat_size)
    ...
    def forward(self, x_in, mask):
        ...
        #compute reconstruction 
        x_rec1=self.proj_feat(x_last, self.hidden_size*2, self.feat_size)
        x_rec2 = self.decoder1(x_rec1)
        x_rec= rearrange(x_rec2, 'b (s1 s2 s3) h w t -> b 1 (h s1) (w s2) (t s3)', s1=32,s2=32,s3=32) 
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_debug(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss(nn.Module):
    def __init__(
        ...
        #super(TransMorph_Unetr, self).__init__()
        super().__init__()
        self.hidden_size = config.embed_dim*2*2*2*2
        self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        #self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        ...
        self.norm_cls = cls_norm(self.hidden_size) # used to miccai
        #self.norm_cls = cls_norm(1536) # used to enlarge the network
        
        self.norm_patch = cls_norm(config.embed_dim*2*2*2) # 
        ...
        #self.decoder1 = nn.Conv3d(self.hidden_size,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1) # used to miccai
        #self.decoder1 = nn.Conv3d(1536,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1) # used to enlarge the network

        self.decoder1 = nn.Conv3d(self.hidden_size,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1)
    ...
    def forward(self, x_in, mask):
        ...
        #compute reconstruction 
        #print('info: x_last ',x_last.shape)
        #print('info: self.hidden_size ',self.hidden_size)
        #print('info: self.feat_size ',self.feat_size)
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_At_Stage_2(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        self.transformer = SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_2(patch_size=config.patch_size, 
                                           in_chans=config.in_chans,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
                                           num_heads=config.num_heads,
        ...
        #self.norm_patch = cls_norm(384) # used to miccai
        #self.norm_patch = cls_norm(768) # used to enlarge the network
        self.norm_patch = cls_norm(192) # used to stage #2
    ...
    def forward(self, x_in, mask):
        ...
        #print ('att_map size ',att_map.shape)
        #print ('x_region_all size ',x_region_all.size())
        return x_region_all ,loss,x_rec,att_map
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_At_Stage_4(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        self.transformer = SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_4(patch_size=config.patch_size, 
                                           in_chans=config.in_chans,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
                                           num_heads=config.num_heads,
        ...
        self.norm_patch = cls_norm(768) # used to stage #4
        self.norm_cls = cls_norm(768) # used to stage #4
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_At_Stage_1(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        self.transformer = SwinTransformer_Unetr_Mask_In_Seperate_only_feature_out_CaiT_All_3_loss_At_Stage_1(patch_size=config.patch_size, 
                                           in_chans=config.in_chans,
                                           embed_dim=config.embed_dim,
                                           depths=config.depths,
                                           num_heads=config.num_heads,
        ...
        #self.norm_patch = cls_norm(768) # used to enlarge the network
        #self.norm_patch = cls_norm(192) # used to stage #2
        self.norm_patch = cls_norm(96) # used to stage #1
    ...
    def get_cls_feature_cvpr(self, x1):

        x1_,x1,_ = self.transformer(x1)
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_w_impainting(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
    ...
    def forward(self, x_in, mask):
        ...
        loss =loss_recon# (loss_recon * mask_intp).sum() / (mask_intp.sum() + 1e-5) 
...
class Trans_SMIT_Only_Patch(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
...
class Trans_SMIT_pre_train_cls_patch_rec_Student_CaiT_All_3_Loss_Patch_Token_At_4_Block(nn.Module):
    ...
    def forward(self, x_in, mask):
        ...
        'Remember here the features are not normalized so normalization is needed'
        'x_feature for patch loss and classfication loss'
        'x_CaiT for [CLS] TOken to mask the image'
        'x_last for image reconstruction loss'
...
class Trans_SMIT_pre_train_only_rec_Student(nn.Module):
    def __init__(
        ...
        super().__init__()
        #self.hidden_size = hidden_size
        self.hidden_size = config.embed_dim*16
        self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        #self.feat_size=(config.img_size[0]//32,config.img_size[1]//32,config.img_size[2]//32)
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
    ...
    def forward(self, x_in, mask):
        ...
        #print('------------------------------>info: x_in size ',x_in.shape)
        x_last = self.transformer(x_in,mask,self.use_MIM_mask)
        
        #becareful of this part
        x_last= self.norm_cls(x_last) 
        #print ('after all transformer x size is ',x_region.size())
        
        #print('------------------------------>info: x_last size ',x_last.shape)
        #print('------------------------------>info: hidden_siz size ',self.hidden_size)
        #print('------------------------------>info: feat_size size ',self.feat_size)
        ...
        if norm_pix_loss:
            ...
            var = x_in.var(dim=-1, keepdim=True)
            x_in = (x_in - mean) / (var + 1.e-6)**.5
...
class Trans_SMIT_pre_train_only_rec_w_Seg_Student(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
        ...
        #self.decoder1 = nn.Conv3d(768,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1)
        self.decoder1 = nn.Conv3d(self.hidden_size,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1)
        self.decoderSeg = nn.Conv3d(self.hidden_size,out_channels=self.encoder_stride ** 3 * 1, kernel_size=1)
    ...
    def forward(self, x_in, mask,x_seg):
        ...
        x_rec= rearrange(x_rec2, 'b (s1 s2 s3) h w t -> b 1 (h s1) (w s2) (t s3)', s1=32,s2=32,s3=32) 
        #x_rec= rearrange(x_rec3, 'b (s1 s2 s3) h w t -> b 1 (h s1) (w s2) (t s3)', s1=32,s2=32,s3=32) 
        x_rec_seg = self.decoderSeg(x_rec1)
        x_rec_seg= rearrange(x_rec_seg, 'b (s1 s2 s3) h w t -> b 1 (h s1) (w s2) (t s3)', s1=32,s2=32,s3=32) 
        ...
        loss=loss+0.5*loss_seg_recon
        #print ('loss rec is  ',loss)
        return loss,x_rec
    
class Trans_SMIT_pre_train_only_rec_Student_in_Painting(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int=1,
